 ...
 * Output: -
 */
void generate_superpixels(IplImage *image, int step_, int nc_) {
    step = step_;
    nc = nc_;
    ns = step_;
	...
	//int img_h = image->height;
	int j, k;
	#pragma omp parallel for num_threads( num_th ) shared( distances, img_w, img_h ) private( j, k ) \
        	default(none) schedule( dynamic, 2 )
        for ( j = 0; j < img_w; j++) {
            for ( k = 0;k < img_h; k++) {
	...
	//printf( "center.size: %d\n", (int)centers.size() ); 
        for (int j = 0; j < (int) centers.size(); j++) {
            /* Only compare to pixels in a 2 x step by 2 x step region. */
	    int center_1 = centers[j][3] + step;
	    int center_2 = centers[j][4] + step;
	    int k, l;
	    //int img_w = image->width;
	    //int img_h = image->height;
	    #pragma omp parallel for num_threads( num_th ) shared( distances, clusters, centers, center_1, center_2, img_w, img_h, j, step, image ) private( k, l ) \
        	default(none) schedule( dynamic, 2 )
            for (k = centers[j][3] - step; k < center_1; k++) {
                for (l = centers[j][4] - step; l < center_2; l++) {
                
                    if (k >= 0 && k < img_w && l >= 0 && l < img_h) {
	...
	int jj, cent_sz = (int) centers.size();;
	#pragma omp parallel for num_threads( num_th ) shared( centers, cent_sz, center_counts ) private( j ) \
        	default(none) schedule( dynamic, 2 )
        for ( jj = 0; jj < cent_sz; jj++) {
            centers[jj][0] = centers[jj][1] = centers[jj][2] = centers[jj][3] = centers[jj][4] = 0;
            center_counts[jj] = 0;
        }
	...
	//int img_h = image->height;
	int kk;
	#pragma omp parallel for num_threads( num_th ) shared( centers, img_w, img_h, clusters, center_counts, image ) private( jj, kk ) \
        	default(none) schedule( dynamic, 2 )
        for ( jj = 0; jj < img_w; jj++) {
            for ( kk = 0; kk < img_h; kk++) {
                int c_id = clusters[jj][kk];
                
                if (c_id != -1) {
                    CvScalar colour = cvGet2D(image, kk, jj);
	...
	int center_sz = (int) centers.size();
	#pragma omp parallel for num_threads( num_th ) shared( centers, center_sz, center_counts ) private( jj ) \
        	default(none) schedule( dynamic, 2 )
        for ( jj = 0; jj < center_sz; jj++) {
            centers[jj][0] /= center_counts[jj];
            centers[jj][1] /= center_counts[jj];
            centers[jj][2] /= center_counts[jj];
            centers[jj][3] /= center_counts[jj];
            centers[jj][4] /= center_counts[jj];
        }
    }
}
...
void create_connectivity(IplImage *image) {
    ...
    int img_h = image->height;
    int i, j, k;
    //#pragma omp parallel for num_threads( num_th ) shared(  img_w, img_h, new_clusters ) private( i, j, k ) \
        	default(none) schedule( dynamic, 2 )
...
int main(int argc, char *argv[]) {
    ...
    display_contours(image, CV_RGB(255,0,0));
